...
EXT_NET_RANGE=${EXT_NET_RANGE:-'start=192.168.1.10,end=192.168.1.90'}
...
if [[ $ENABLE_EXT_NET -eq 1 ]]; then
   ...
   $KOLLA_OPENSTACK_COMMAND subnet create --dhcp --allocation-pool ${EXT_NET_RANGE} --network public1 --subnet-range ${EXT_NET_CIDR} --gateway ${EXT_NET_GATEWAY} --dns-nameserver 8.8.8.8 public1-subnet
